...
impl<K, V> SkipList<K, V> {
    ...
    where
        ...
        V: 'static,
    {
        // SAFETY: this function requires `K: 'static` and `V: 'static` bounds.
        unsafe { Self::new_unchecked(collector) }
    }
    ...
    ///
    /// The easiest way to ensure this is to require `K: 'static` and `V: 'static`
    /// bounds, like [`new`](Self::new).
    pub unsafe fn new_unchecked(collector: Collector) -> Self {
        // SAFETY: the caller must uphold the safety contract.
        unsafe { Self::with_comparator_unchecked(collector, Default::default()) }
    }
}
...
impl<K, V, C> SkipList<K, V, C> {
    ...
    {
        // SAFETY: this function requires `K: 'static` and `V: 'static` bounds.
        unsafe { Self::with_comparator_unchecked(collector, comparator) }
    }
    ...
    ///
    /// The easiest way to ensure this is to require `K: 'static` and `V: 'static`
    /// bounds, like [`with_comparator`](Self::with_comparator).
    pub unsafe fn with_comparator_unchecked(collector: Collector, comparator: C) -> Self {
        Self {
            head: Head::new(),
            collector,
