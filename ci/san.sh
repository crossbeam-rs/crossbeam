...
cargo clean
RUSTFLAGS="-Dwarnings -Z sanitizer=address --cfg crossbeam_sanitize" \
...
cargo clean
RUSTFLAGS="-Dwarnings -Z sanitizer=memory --cfg crossbeam_sanitize" \
...
RUSTFLAGS="-Dwarnings -Z sanitizer=thread --cfg crossbeam_sanitize" \
    cargo test -Z build-std --all --release --target x86_64-unknown-linux-gnu --tests --exclude benchmarks -- --test-threads=1
